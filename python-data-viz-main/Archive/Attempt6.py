...
def sum_infections():
    ...
    data_by_year = data.groupby('Year')['Flock Size'].sum().to_dict()
    return total_infections, data_by_state, data_by_year

data_summary = sum_infections()
total_infections = data_summary[0]
infections_by_state = data_summary[1]
infections_by_year = data_summary[2]
...
def infections_by_time():
    ...
    count = infections_by_year.get(int(year), 0) if year.isdigit() else 0
    return jsonify(infections_by_time=count)
...
def time_series():
    fig = px.line(data.groupby('Year')['Flock Size'].sum().reset_index(),
                  x='Year', y='Flock Size', title='Infections Over Time',
                  labels={'Flock Size': 'Total Infections'})
